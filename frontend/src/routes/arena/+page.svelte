<script lang="ts">
  ...
  // Keep a pane pinned to the latest streamed text, unless the reader has
  // scrolled up in it (then it stops following until they scroll back down).
  function autoscroll(node: HTMLElement, _text: string) {
    let stick = true;
    const onScroll = () => {
      stick = node.scrollHeight - node.scrollTop - node.clientHeight < 80;
    };
    ...
    <div class="panes">
      {#each [{ side: 'A', model: modelA, text: textA }, { side: 'B', model: modelB, text: textB }] as p (p.side)}
        <section class="pane">
          ...
          <div class="pane-body" use:autoscroll={p.text}>
            {#if p.text}
              <Markdown source={p.text} reasoning />
            {:else if streaming}
              <div class="thinking" role="status" aria-label="generating…" title="generating…">
                <span class="dot"></span><span class="dot"></span><span class="dot"></span>
              </div>
            {:else}
              <div class="pane-empty">enter a prompt below to start a battle</div>
            {/if}
          </div>
        </section>
      {/each}
...
<style>
  ...
  .thinking { display: inline-flex; gap: 0.3rem; margin: auto; }
  .thinking .dot {
    width: 0.5rem;
    height: 0.5rem;
    border-radius: 50%;
    ...
    opacity: 0.45;
    animation: arena-think 1.2s ease-in-out infinite;
  }
  .thinking .dot:nth-child(2) { animation-delay: 0.2s; }
  .thinking .dot:nth-child(3) { animation-delay: 0.4s; }
  @keyframes arena-think {
    0%, 80%, 100% { opacity: 0.3; transform: translateY(0); }
    40% { opacity: 1; transform: translateY(-0.2rem); }
  }
  @media (prefers-reduced-motion: reduce) {
    .thinking .dot { animation: arena-fade 1.2s ease-in-out infinite; }
    @keyframes arena-fade { 0%, 100% { opacity: 0.3; } 50% { opacity: 1; } }
  }
  .vote {
    display: flex;
    gap: 0.5rem;
